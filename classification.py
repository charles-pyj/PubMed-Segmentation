...
def classification(args):
    ...
    json_in = read_json_processed(args.input_json)
    predicted_classes = batched_inference(classifyModel,json_in,batch=32,mode="url")
    with open("./single_panel_classes.json",'w') as f:
...
def get_single_panel_image(args):
    json_in = read_json_processed(args.input_json)
    segmodel = load_segmentation_model(args.segment_model_path)
    max_length = 200000
    panel_cnts = []
    for i in tqdm(range(max_length)):
        print(json_in[i])
        panel_cnts.append(len(seg_url(segmodel,json_in[i])["segments"]))
    with open("./panel_cnt.json","w") as f:
        json.dump(panel_cnts,f)

def get_single_panel_image_class(args):
    json_in = read_json_processed(args.input_json)
    segmodel = load_segmentation_model(args.segment_model_path)
    max_length = 200000
    panel_cnts = []
...
def main():
    ...
    parser.add_argument('--input_json', type=str, required=True, help='Input directories, containing one or multiple json files')

    args = parser.parse_args()
    #print(len(read_json_processed(args.input_json)))
    classification(args)
